...
class _ChangeCarLocationPageWidgetState
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      safeSetState(() {});
      _model.branchSearchData = BranchDataTypeStruct();
      safeSetState(() {});
      ...
      if (widget!.fromPage != 'takeImages') {
        ...
        if (widget!.impoundCarLocateParamSet?.impoundLocatBranchCode != '') {
          _model.improundCarSearchBranchOutput =
              await ImproundCarSearchBranchCall.call(
          ...
          if ((_model.improundCarSearchBranchOutput?.statusCode ?? 200) !=
              200) {
            ...
            await showDialog(
              ...
              builder: (alertDialogContext) {
                return WebViewAware(
                  child: AlertDialog(
                    content: Text(
                        'พบข้อผิดพลาด Connection(${(_model.improundCarSearchBranchOutput?.statusCode ?? 200).toString()})'),
                    actions: [
                      TextButton(
          ...
          if (ImproundCarSearchBranchCall.statusLayer1(
                (_model.improundCarSearchBranchOutput?.jsonBody ?? ''),
              ) !=
              '200') {
            Navigator.pop(context);
            await showDialog(
              ...
              builder: (alertDialogContext) {
                return WebViewAware(
                  child: AlertDialog(
                    content: Text('${ImproundCarSearchBranchCall.messageLayer1(
                      (_model.improundCarSearchBranchOutput?.jsonBody ?? ''),
                    )}'),
                    actions: [
                      TextButton(
          ...
          _model.branchSearchData = ImproundCarSearchBranchCall.data(
            (_model.improundCarSearchBranchOutput?.jsonBody ?? ''),
          );
          safeSetState(() {});
        }
                                  ...
                                  if (((FFAppState().profileLevel == 'สาขา') ||
                                          ...
                                          ? ((functions.containsValueInJsonList(
                                                  ...
                                                  (functions.getListStringFromJson(functions.getDataFromMapJson(functions.getDataFromMapJson(widget!.editAccessRoleData, widget!.step), 'position_name'))!.length > 0
                                                      ? functions.containsValueInJsonList(
                                                          functions.getDataFromMapJson(functions.getDataFromMapJson(widget!.editAccessRoleData, widget!.step), 'position_name'),
                                                          FFAppState().profilePositionName)!
                                                      : true)) &&
                                              (widget!.step == 'step3' ? ((_model.branchSearchData == BranchDataTypeStruct() ? false : ((_model.branchSearchData?.areaCode == FFAppState().profileBranch) || (_model.branchSearchData?.regionCode == FFAppState().profileBranch))) || widget!.regionCodeList!.contains(FFAppState().profileBranch)) : true))
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (widget!.fromPage == 'takeImages') {
                                      ...
                                      _model.updateCarLocationOutput =
                                        ...
                                        impoundLocatBranchCode: widget!.step ==
                                                'step3'
                                            ? (_model.allowChangeLocation &&
                                                ...
                                                : '')
                                            : '',
                                        impoundLocatBranchName: widget!.step ==
                                                'step3'
                                            ? (_model.allowChangeLocation &&
                                                    (_model.dropDownValue ==
                                                        'L001')
